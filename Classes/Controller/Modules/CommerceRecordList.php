 ...
 *
 * Class \CommerceTeam\Commerce\Modules\CommerceRecordList
 *
 * @author 2005-2012 Franz Holzinger <kontakt@fholzinger.com>
 */
class CommerceRecordList extends \TYPO3\CMS\Recordlist\RecordList\DatabaseRecordList
{
    /**
     * Parent uid.
     *
     ...
     */
    public function getButtons_ORIG()
    {
    	$row = func_get_args();
		if(! empty($row)) {
			$row = $row[0];
     ...
     */
    public function getTable_ORIG($table, $id, $rowlist = '')
    {
     ...
     */
    public function makeQueryArray_ORIG($table, $id, $addWhere = '', $fieldList = '*')
    {
        $database = $this->getDatabaseConnection();
     ...
     */
    public function addElement_ORIG($h, $icon, $data, $trParams = '', $lMargin = '', $altLine = '', $colType = 'td')
    {
     ...
     * @throws \UnexpectedValueException If hook was of wrong interface
     */
    public function renderListHeader_ORIG($table, $currentIdList)
    {
        $language = $this->getLanguageService();
        $tableConfig = SettingsFactory::getInstance()->getTcaValue($table);
     ...
     * @throws \UnexpectedValueException If hook was of wrong interface
     */
    public function makeControl_ORIG($table, $row)
    {
        $backendUser = $this->getBackendUser();
        $language = $this->getLanguageService();
        $tableConfig = SettingsFactory::getInstance()->getTcaValue($table);
     ...
     * @throws \UnexpectedValueException If hook was of wrong interface
     */
    public function makeClip_ORIG($table, $row)
    {
        $language = $this->getLanguageService();
     ...
     */
    public function makeLocalizationPanel_ORIG($table, $row)
    {
        $backendUser = $this->getBackendUser();
     ...
     * @return string URL
     */
    public function listURL_ORIG($altId = '', $table = '-1', $excludeList = '')
    {
        $urlParameters = array();
        if (strcmp($altId, '')) {
     ...
     */
    public function linkWrapItems_ORIG($table, $uid, $code, $row)
    {
        $language = $this->getLanguageService();
        $backendUser = $this->getBackendUser();
